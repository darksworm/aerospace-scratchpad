...
func TestAeroSpaceQuerier(t *testing.T) {
	...
	t.Run("GetFilteredWindows returns two matches with pattern only", func(t *testing.T) {
		ctrl := gomock.NewController(t)
		defer ctrl.Finish()

		tree := []testutils.AeroSpaceTree{{
			Windows: []aerospacecli.Window{
				{AppName: "Finder1", WindowID: 1, WindowTitle: "Finder - foo", AppBundleID: "com.apple.finder"},
				{AppName: "Finder2", WindowID: 2, WindowTitle: "Finder2 - bar", AppBundleID: "com.apple.finder"},
				{AppName: "Terminal", WindowID: 3, WindowTitle: "Terminal", AppBundleID: "com.apple.terminal"},
			},
		...
		if err != nil || len(wins) != 2 {
			t.Fatalf("expected 2 finder windows, got %d err=%v", len(wins), err)
		}
	})
	...
	t.Run("GetFilteredWindows with filters narrows to one", func(t *testing.T) {
		...
		client.EXPECT().
			...
			Times(1)
		q := NewAerospaceQuerier(client)
		wins, err := q.GetFilteredWindows("Finder", []string{"window-title=foo", "app-bundle-id=apple"})
	...
	t.Run("GetFilteredWindows unknown filter property returns error", func(t *testing.T) {
		ctrl := gomock.NewController(t)
		defer ctrl.Finish()
	...
	t.Run("GetFilteredWindows with pattern only and no matches returns error", func(t *testing.T) {
		ctrl := gomock.NewController(t)
		defer ctrl.Finish()
	...
	t.Run("GetFilteredWindows returns error when GetAllWindows fails", func(t *testing.T) {
		ctrl := gomock.NewController(t)
		defer ctrl.Finish()
